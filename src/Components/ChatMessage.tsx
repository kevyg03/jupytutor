...
export const ChatMessage = (props: ChatHistoryItem) => {
    const message = typeof props.content === 'string' ? props.content : props.content[0].text;
...
const AssistantMessage = (props: AssistantMessageProps): JSX.Element => {
    ...
    return (
        <div className={`assistant-message ${isVisible ? 'assistant-visible' : ''} ${streaming === 'streaming' ? 'assistant-streaming' : ''}`}>
            <ReactMarkdown
                components={{
                    a: ({ node, ...props }) => (
                        <a 
...
export const StreamingAssistantMessage = (props : StreamingAssistantMessageProps): JSX.Element | null => {
